...
1. hashcode
2. equals
3. getClass：返回当前运行时的类
4. toString：返回对象用字符串表示
5. notify：唤醒再此对象等待的线程
6. notifyall：唤醒在此对象上等待的所有线程
7. wait(long timeout)：使当前对象的线程等待timeout时长
8. wait(long timeout,int nanos)：使当前对象线程等待timeout时长，或者其他线程中断当前线程
9. wait()：使当前线程等待
## ==和equal的区别
...
2. stringbuffer是可以被修改的，并且有synchronize修饰，是线程安全的
3. stringbuild也会是可以被修改的，是线程不安全的，具有高性能
## 什么是反射
...
## 空指针异常
## int和interger的区别
1. int是基本数据类型，Interger是引用类型个
2. int的默认值是0，Interger的默认值是null
3. int的类型可以直接存储，Interger需要进行实例化
## 耦合类型
1. 内容耦合：
2. 公共耦合：一组模块都访问同一个数据环境
3. 外部耦合： 一组模块都访问同一个全局简单变量
4. 控制耦合： 一个模块通过开关什么的明显控制另一个模块的功能叫做控制耦合
5. 数据耦合：一个模块访问另一个模块时，彼此之间通过简单地数据参数来交换输入，输出信息的
6. 非直接耦合：两个模块之间的联系完全是通过主模块的控制和调用来实现的
7. 标记耦合
